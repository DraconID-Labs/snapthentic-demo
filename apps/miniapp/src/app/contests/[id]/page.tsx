...
export default function ContestPage() {
  ...
  const { data: contest, isLoading: contestLoading } = api.contests.getById.useQuery(
    { id: contestId },
    { enabled: !!contestId }
  );

  const { data: entries, isLoading: entriesLoading, refetch: refetchEntries } = 
    api.contests.getEntries.useQuery(
      { contestId },
      { enabled: !!contestId }
    );
  ...
  if (!contest) {
    return <div className="flex min-h-screen items-center justify-center">Contest not found</div>;
  }
      ...
      <div className="mt-6 w-full max-w-4xl">
        ...
        {entries?.length === 0 ? (
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 lg:grid-cols-3">
            {entries?.map((entry) => (
              <div key={entry.id} className="relative overflow-hidden rounded-lg bg-white shadow-md">
                <ResponsiveImage
                ...
                <div className="absolute inset-x-0 bottom-0 bg-gradient-to-t from-black/70 to-transparent p-4">
                  <div className="text-white">
                    ...
                    <p className="text-sm opacity-90">{entry.snap.description}</p>
                    <p className="text-xs opacity-75">by @{entry.snap.author.displayName}</p>
                  </div>
                </div>
                ...
                <div className="absolute right-2 top-2 flex items-center gap-2">
                  ...
                  >
                    {voteStatus?.[entry.id] ? <HeartOff size={16} /> : <Heart size={16} />}
                  </Button>
                </div>
              </div>
            ))}
